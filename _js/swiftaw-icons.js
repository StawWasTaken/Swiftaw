...
(function () {
  ...
  /* A shelf's mark. Which one that is has already been decided by the database:
     the one somebody chose, or failing that the first icon filed on the shelf,
     which is nearly always the right picture because the first thing on a shelf
     is what somebody thought the shelf was for. mark_body carries the answer.

     Only a shelf that is both unmarked and empty falls through to its initial
     on a tile, so that reads as a decision nobody has made yet rather than as a
  ...
  function browse() {
    ...
    window.SwiftawAccount.ready(function () {
      ...
      // Not a plain select: this one resolves a bare shelf's borrowed mark, and
      // comes back already in order.
  ...
  function manage() {
    ...
    /* Three states, and the line has to tell them apart. A shelf borrowing its
       first icon is not bare: it has a picture, just not one anybody chose. A
       shelf that is bare has nothing at all. Counting them together would say a
       shelf needs attention when it does not, or hide one that does. */
    function paintCats() {
