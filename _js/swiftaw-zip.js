/* SwiftawZip - packs a handful of files into one .zip, in the browser.
 *
 *   SwiftawZip([{ name: 'folder/file.png', data: Uint8Array | string }]) -> Blob
 *
 * Stored, not deflated. Everything we hand out is a PNG, which is already
 * compressed, so deflating it again buys a percent or two and costs a whole
 * compressor.
 */
(function (w) {
  'use strict';
  ...
  /* MS-DOS packed date and time. The zip format has no room for anything
     better, and a wrong-looking timestamp is worse than a fixed one. */
  function stamp() {
    var d = new Date();
    return {
  ...
  function zip(entries) {
    ...
    items.forEach(function (it) {
      ...
      /* Bit 11 says the name is UTF-8. Ours are ASCII, but a reader that
         honours the flag then never has to guess a code page. */
